        ...
        if (!is_wp_error($response)) {
            ...
            if (isset($data['access_token'])) {
                ...
                if ($post_type == 'GET') {
                    ...
                    if (!empty($body_data)) {
                        $endpoint_url = add_query_arg($body_data, $endpoint_url);
                    }
    ...
    if (isset($_GET['code'])) {
        ...
        if (!is_wp_error($response)) {
            ...
            if (isset($data->access_token)) {
                ...
                $original_url = home_url().'/display-profiles/';
                ...
                if ($post_type == 'GET') {
                    ...
                    $body_data = isset($state['body_data']) ? $state['body_data'] : array();
                    $response = wp_remote_put($endpoint_url, array(
                        'headers' => array(
                            'Authorization' => 'Bearer ' . $access_token,
                            'Content-Type' => 'application/json', // Set content type to JSON
                        ),
...
function redirect_to_authorization_url_06($params) {
