...
// no use
function redirect_to_authorization_url_2() {
    $tenant_id = get_option('tenant_id');
    $client_id = get_option('client_id');
    $redirect_uri = get_option('redirect_uri');
    $scope = array('https://api.businesscentral.dynamics.com/.default');
    ...
    $authorization_params = array(
        ...
        'response_type' => 'code',
        'redirect_uri' => $redirect_uri,
        'scope' => implode(' ', $scope),  // Join array elements into a space-separated string
        'response_mode' => 'query',
        'state' => '12345',  // Use a more secure and random state in production
    );
    
    // Use http_build_query to handle array parameters
    $authorization_url = $authorize_url . '?' . http_build_query($authorization_params);
...
// Trigger the redirect when needed, e.g., on a specific page load
add_action('template_redirect', 'redirect_to_authorization_url_2');
...
function handle_oauth_callback_2() {
    ...
    if (isset($_GET['code'])) {
        ...
        // Retrieve OAuth 2.0 settings from the options table
        $tenant_id = get_option('tenant_id');
        $client_id = get_option('client_id');
        $client_secret = get_option('client_secret');
        $redirect_uri = get_option('redirect_uri');
        ...
        // Token endpoint to exchange authorization code for access token
        $token_endpoint = "https://login.microsoftonline.com/$tenant_id/oauth2/v2.0/token";
        $response = wp_remote_post($token_endpoint, array(
            'body' => array(
                'client_id' => $client_id,
        ...
        if (!is_wp_error($response)) {
            ...
            if (isset($data->access_token)) {
                ...
                update_option('bc_access_token', $access_token);
