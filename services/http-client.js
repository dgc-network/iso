...
/*    
        ...
        mqttClientInit.on('message', function (topic, message) {
            ...
            Object.keys(parsedMessage).forEach(key => {
                if (keyMapping[key] !== undefined) {
                    ...
                    update_http_client_data(topic, keyMapping[key], parsedMessage[key]);
                }
            });
    ...
    function update_http_client_data(topic, key, value) {
        console.log(`Updating ${key} for topic ${topic} with value ${value}`);
        jQuery.ajax({
            type: 'POST',
            url: ajax_object.ajax_url,
            data: {
                action: 'update_http_client_data',
                _topic: topic,
                _key: key,
                _value: value,
            },
